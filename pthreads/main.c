...
void *dissipate_cells(void *parameters) {
    ...
    double difference = params->epsilon + 1;

    while(difference > params->epsilon) {
        ...
        for(int index = params->startIndex; index < (params->startIndex + params->numBoxes); index++) {
            params->boxes[index].dsv = params->temp_dsv[index];
        }
        pthread_barrier_wait(&barrier);

        //Run the update section one thread at a time
        pthread_mutex_lock(&lock);
        params->results[1] = params->boxes[0].dsv;
        params->results[2] = params->boxes[0].dsv;
        for(int i = 0; i < params->num_containers; i++) {
            if(params->boxes[i].dsv > params->results[1]) {
                params->results[1] = params->boxes[i].dsv;
            }
            if(params->boxes[i].dsv < params->results[2]) {
                params->results[2] = params->boxes[i].dsv;
            }
        }
        params->results[0]++;
        difference = (params->results[1] - params->results[2]) / params->results[1];
...
void amr_dissipation(int num_containers, struct box boxes[], double results[], double temp_dsv[], float epsilon, float affect_rate, int num_threads) {
    ...
    for(int i = 0, startIndex = 0; i < num_threads; i++, startIndex += blockSize ) {
        ...
        params[i].results = &results[0];
        params[i].epsilon = epsilon;
        params[i].num_threads = num_threads;
        params[i].num_containers = num_containers;
    }
    //add extra boxes to last thread
    params[num_threads - 1].numBoxes += num_containers - (blockSize * num_threads);
    pthread_barrier_init(&barrier, &barrier_attr, params->num_threads);
...
int main(int argc, char *argv[]) {
    ...
    {
        amr_dissipation(num_containers, boxes, results, temp_dsv, epsilon, affect_rate, num_threads);
    }
