...
void amr_dissipation(int num_containers, struct box boxes[], double results[], double difference, double temp_dsv[], float epsilon, float affect_rate, int num_threads) {
    while(difference > epsilon) {
        ...
        #pragma omp parallel
        {
            ...
            for (int index = id * blockSize; index < (id + 1) * blockSize; index++) {
                ...
                if (boxes[index].top_neighbors > 0) {
                    for (int i = 0; i < boxes[index].top_neighbors; i++) {
                        begin_overlap = max(boxes[index].upper_left_x,
                ...
                if (boxes[index].bottom_neighbors > 0) {
                    for (int i = 0; i < boxes[index].bottom_neighbors; i++) {
                        begin_overlap = max(boxes[index].upper_left_x,
                                            boxes[boxes[index].bottom_neighbors_list[i]].upper_left_x);
                        end_overlap = min(boxes[index].upper_left_x + boxes[index].width,
                                          boxes[boxes[index].bottom_neighbors_list[i]].upper_left_x +
                                          boxes[boxes[index].bottom_neighbors_list[i]].width);
                        overlap = end_overlap - begin_overlap;
                ...
                if (boxes[index].left_neighbors > 0) {
                    for (int i = 0; i < boxes[index].left_neighbors; i++) {
                        begin_overlap = max(boxes[index].upper_left_y,
                                            boxes[boxes[index].left_neighbors_list[i]].upper_left_y);
                        end_overlap = min(boxes[index].upper_left_y + boxes[index].height,
                                          boxes[boxes[index].left_neighbors_list[i]].upper_left_y +
                                          boxes[boxes[index].left_neighbors_list[i]].height);
                        overlap = end_overlap - begin_overlap;
                ...
                if (boxes[index].right_neighbors > 0) {
                    for (int i = 0; i < boxes[index].right_neighbors; i++) {
                        begin_overlap = max(boxes[index].upper_left_y,
                                            boxes[boxes[index].right_neighbors_list[i]].upper_left_y);
                        end_overlap = min(boxes[index].upper_left_y + boxes[index].height,
                                          boxes[boxes[index].right_neighbors_list[i]].upper_left_y +
                                          boxes[boxes[index].right_neighbors_list[i]].height);
                        overlap = end_overlap - begin_overlap;
            ...
            //collect all threads after updating values
            #pragma omp barrier

            //Commit new data - master thread only
            #pragma omp master
            {
            ...
            //Ensure that the master thread joins up properly
            #pragma omp barrier
        }
    }
}
